...
static
BYTE rcvr_data (
	BYTE addr,		/* Address of the register to read */
...
)
{
	...
	BYTE token = 0x80 | addr;				/* Read token */
	xmit_sensor(token);
...
static
SSTATUS xmit_data (
	BYTE out,		/* Data buffer which contains data to be written */
	BYTE addr,		/* Address of the register to write */
	SENSOR sensor	/* Sensor type */
)
{
	switch(sensor){
	...
	BYTE token = 0x7F & addr;		/* Write token */
	xmit_sensor(token);
	xmit_sensor(out);
	...
	BYTE sample = rcvr_data(addr, sensor);
